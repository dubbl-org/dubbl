...
export default function PODetailPage() {
  ...
  async function handleConvert() {
    ...
    if (res.ok) { const data = await res.json(); toast.success("Converted to bill"); router.push(`/purchases/${data.bill.id}`); }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={po.poNumber} description={`From: ${po.contact?.name || "Unknown"}`}>
        <Button variant="outline" size="sm" asChild><Link href="/purchases/orders"><ArrowLeft className="mr-2 size-4" />Back</Link></Button>
