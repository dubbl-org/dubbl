...
export default function QuotesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/quotes?${params}`, { headers: { "x-organization-id": orgId } })
      .then((r) => r.json())
      .then((data) => { if (data.data) setQuotes(data.data); })
  ...
  if (!loading && quotes.length === 0 && statusFilter === "all") {
    return (
      <div className="space-y-6">
        ...
        <EmptyState icon={FileText} title="No quotes yet" description="Create your first quote to send to customers.">
          <Button onClick={() => router.push("/quotes/new")} className="bg-emerald-600 hover:bg-emerald-700"><Plus className="mr-2 size-4" />New Quote</Button>
        </EmptyState>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable columns={columns} data={quotes} loading={loading} emptyMessage="No quotes found." onRowClick={(r) => router.push(`/quotes/${r.id}`)} />
    </div>
  );
}
