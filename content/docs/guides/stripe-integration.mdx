...
3. Authorize dubbl in the Stripe OAuth flow
4. Dubbl automatically creates the required chart accounts and begins an initial sync

<Callout type="info">
  The initial sync imports customers and the last 30 days of charges and payouts. You can adjust this in integration settings.
</Callout>

### Account Mappings

When you connect, dubbl auto-creates these accounts if they don't exist:

| Account | Code | Type | Purpose |
|---------|------|------|---------|
| Stripe Clearing | `1350` | Asset (Current) | Holds funds between charge and payout |
| Payment Processing Fees | `6150` | Expense | Stripe's per-transaction fees |
| Revenue | `4100` | Revenue | Where charge amounts are credited (uses existing) |

You can change the account mappings at any time from the integration settings page.
...
4. Connected account webhooks are registered automatically during OAuth
...
6. Dubbl automatically marks the invoice as paid and creates a journal entry (DR Bank, CR Accounts Receivable)
...
| Business | $29/seat/mo | $24/seat/mo (billed annually) |
